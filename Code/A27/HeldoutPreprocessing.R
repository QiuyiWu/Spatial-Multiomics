...
############ Metabolomics Data #########
######################################## 
a27.met.data <- read_excel("../data/PostCombat_A27_Metabolite_beforeHeldout.xlsx")
...
pca_met_corrected <- prcomp(a27.met.data[,-c(1:3)], scale. = TRUE)
...
ggplot(pca_met, aes(x=PC1, y=PC2, colour = plate  )) + #, color=color_indicator)) +
  geom_point(size=2) +
  geom_text_repel(data=outlier_met, aes(label=sample), size=3, color="black") +  # Add outlier labels
  labs(title="PCA Plot of a27 Metabolomics Data using [ComBat]",
       x="PC 1",
       y="PC 2") +
  theme_minimal() +
...
a27.pro.data <- read_excel("../data/PostCombat_a27_Protein.xlsx", sheet = "combat_corrected_unscaled")

# PCA + PLOT
pca_pro <- prcomp(a27.pro.data[,-c(1:3)], scale. = TRUE)
pca_pro <- data.frame(pca_pro$x)  # PCA scores
pca_pro$plate <- a27.pro.data$pro.plate
pca_pro$sample_type <- a27.pro.data$pro.sampletype
pca_pro$color_indicator <- ifelse(pca_pro$sample_type == "SPQC", "black", pca_pro$plate)
pca_pro$sample <- a27.pro.data$pro.sample
...
sum(paste0("ID", dukeid.df2$DukeID[!is.na(dukeid.df2$DukeID)]) %in% a27.pro.data$pro.sample)
sum( dukeid.df2$AnalysisID[!is.na(dukeid.df2$AnalysisID)] %in% as.numeric(a27.met.data$Sample))
...
colnames(pca_pro)[ncol(pca_pro)] <- "DukeID"
...
ggplot(pca_pro, aes(x=PC1, y=PC2, color=color_indicator)) +
  ...
  labs(title="PCA Plot of a27 proabolome Data using [ComBat]",
       x="PC 1",
       y="PC 2") +
  theme_minimal() +
  scale_color_manual(values=c("black", "red", "blue", "green", "purple", "orange", "pink")) +  # Set black for SPQC
...
# 546 common samples in both datasets
...
# HELDOUT
heldout.df <- data.frame(read_excel("../data/A27_Frizzle_Flatmap_Grid_20260317.xlsx", sheet = "Decoder"))
heldout.df <- heldout.df[, c("AnalysisID", "SplitAnalysisID")]
colnames(heldout.df) <- c("Mapped_AnalysisID", "HeldOut_AnalysisID1")
...
# library(openxlsx)
# write.xlsx(k_table_wide,"Heldout_Updated_a27.xlsx")
